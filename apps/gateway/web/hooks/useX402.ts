import { readContract, writeContract } from '@jejunetwork/contracts'
...
export function useX402(): UseX402Return {
  ...
  const pay = useCallback(
    async (
      ...
      try {
        if (!walletClient || !address) throw new Error('Wallet not connected')
        const settleHash = await writeContract(walletClient, {
        ...
        const logsWithTopics = receipt.logs as Array<{
        ...
        setPaymentState({
          ...
          error: null,
          txHash: settleHash,
          paymentId: (logsWithTopics[0]?.topics[1] || '0x') as Hex,
        })
