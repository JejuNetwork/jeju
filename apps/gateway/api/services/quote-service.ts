...
import { SOLVER_REGISTRY_ADDRESS } from '../../lib/config/contracts'
import { getRpcUrl, IS_TESTNET } from '../../lib/config/networks'
...
/** Parse solver addresses from environment, validating each one */
const envSolvers: Address[] = (process.env.OIF_DEV_SOLVER_ADDRESSES || '')
  .split(',')
  .filter((addr): addr is Address => isAddress(addr))
  .map((addr) => getAddress(addr))

const discoveredSolvers = new Set<Address>()
let lastDiscoveryBlock = 0n
const DISCOVERY_INTERVAL_MS = 60_000
let lastDiscoveryTime = 0

async function discoverSolvers(): Promise<Address[]> {
...
async function fetchSolverInfo(address: Address): Promise<SolverInfo | null> {
