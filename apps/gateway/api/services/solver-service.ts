...
import { initializeState, solverState } from './state'

/** Parse and validate solver addresses from environment */
const KNOWN_SOLVER_ADDRESSES: Address[] = (
  process.env.OIF_DEV_SOLVER_ADDRESSES || ''
)
  .split(',')
  .filter((addr): addr is Address => isAddress(addr))
...
export class SolverService {
  ...
  private async syncKnownSolvers(): Promise<void> {
    for (const address of KNOWN_SOLVER_ADDRESSES) {
      await this.refreshSolverFromChain(address)
    }
  }
  ...
  private async refreshSolverFromChain(
    ...
    if (chainInfo?.isActive) {
      ...
      // Filter to only supported chain IDs
      const supportedChains: SupportedChainId[] = chainInfo.supportedChains
        .map((c) => Number(c))
        .filter((chainId): chainId is SupportedChainId =>
          isSupportedChainId(chainId as SupportedChainId),
        )
  ...
  async listSolvers(params?: {
    ...
    let solvers = await solverState.list({
      ...
      minReputation: validated?.minReputation ?? undefined,
    })
  ...
  async findSolversForRoute(
    ...
    return allSolvers.filter((solver) => {
      const supportsSource = solver.supportedChains.includes(
        validatedSourceChain as SupportedChainId,
      )
      const supportsDest = solver.supportedChains.includes(
        validatedDestChain as SupportedChainId,
      )
