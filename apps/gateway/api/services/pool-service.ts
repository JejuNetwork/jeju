...
import { SwapQuoteRequestSchema, TokenPairSchema } from '../../lib/validation'

const MAX_POOLS_TO_FETCH = 100

const CHAINLINK_ETH_USD_FEEDS: Record<number, Address> = {
  1: getAddress('0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419'),
  42161: getAddress('0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612'),
  10: getAddress('0x13e3Ee699D1909E989722E753853AE30b17e08c5'),
}
...
> = {
  ...
  WETH: {
    address: parseEnvAddress(process.env.WETH_ADDRESS, ZERO_ADDRESS),
    symbol: 'WETH',
    decimals: 18,
  },
  USDC: {
    address: parseEnvAddress(
...
class PoolService {
  ...
  private contracts = {
    v2Factory: parseEnvAddress(process.env.XLP_V2_FACTORY, ZERO_ADDRESS),
    v3Factory: parseEnvAddress(process.env.XLP_V3_FACTORY, ZERO_ADDRESS),
    router: parseEnvAddress(process.env.XLP_ROUTER, ZERO_ADDRESS),
    aggregator: parseEnvAddress(process.env.XLP_AGGREGATOR, ZERO_ADDRESS),
    paymaster: parseEnvAddress(process.env.CROSS_CHAIN_PAYMASTER, ZERO_ADDRESS),
  }
  ...
  /** Type-safe wrapper for contract reads */
  private read<T>(params: {
    address: Address
    abi: readonly unknown[]
    functionName: string
    args?: readonly unknown[]
  }): Promise<T> {
    return safeReadContract<T>(this.client, params)
  }
  ...
  private async getETHPriceUSD(): Promise<bigint> {
    ...
    if (ethMainnetFeed) {
      try {
        ...
        const data = await safeReadContract<
  ...
  async listV2Pools(): Promise<V2Pool[]> {
    ...
    const pairCount = await this.read<bigint>({
    ...
    for (let i = 0; i < Math.min(count, MAX_POOLS_TO_FETCH); i++) {
      const pairAddress = await this.read<Address>({
        address: this.contracts.v2Factory,
        abi: V2_FACTORY_ABI,
  ...
  async getV3Pool(
    ...
    const poolAddress = await this.read<Address>({
  ...
  async listPoolsForPair(token0: Address, token1: Address): Promise<Pool[]> {
    ...
    if (this.contracts.v2Factory !== ZERO_ADDRESS) {
      const v2Pair = await this.read<Address>({
        address: this.contracts.v2Factory,
        abi: V2_FACTORY_ABI,
        functionName: 'getPair',
        args: [validated.token0, validated.token1],
      }).catch((error): null => {
    ...
    if (this.contracts.paymaster !== ZERO_ADDRESS) {
      const reserves = await this.read<readonly [bigint, bigint]>({
  ...
  async getSwapQuote(
    ...
    if (this.contracts.aggregator !== ZERO_ADDRESS) {
      const quote = await this.read<{
        ...
        args: [validated.tokenIn, validated.tokenOut, amountInWei],
      }).catch((error): null => {
  ...
  async getAllSwapQuotes(
    ...
    if (this.contracts.aggregator !== ZERO_ADDRESS) {
      const allQuotes = await this.read<
        readonly {
          poolType: number
          pool: Address
          amountOut: bigint
  ...
  async getPoolStats(): Promise<PoolStats> {
    ...
    if (this.contracts.paymaster !== ZERO_ADDRESS) {
      const stats = await this.read<readonly [bigint, bigint]>({
        address: this.contracts.paymaster,
        abi: PAYMASTER_AMM_ABI,
