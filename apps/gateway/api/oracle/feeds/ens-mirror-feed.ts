...
export class ENSMirrorFeed {
  ...
  async fetchENSState(name: string): Promise<ENSState | null> {
    ...
    // Get resolver
    const resolverAddr = await readContract(this.ethClient, {
    ...
    for (const key of textKeys) {
      const value = await readContract(this.ethClient, {
        address: resolverAddr,
        abi: ENS_RESOLVER_ABI,
        functionName: 'text',
        args: [node, key],
...
export function createENSMirrorFeed(
  ...
  return new ENSMirrorFeed({
    ...
    oraclePrivateKey: expectHex(
      config?.oraclePrivateKey ?? process.env.ORACLE_PRIVATE_KEY ?? '0x0',
    ),
