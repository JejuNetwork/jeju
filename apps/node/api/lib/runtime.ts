...
/** Staking contract ABI for staking queries */
const STAKING_QUERY_ABI = [
  {
    name: 'getStake',
    type: 'function',
    stateMutability: 'view',
...
function createBrowserAPI(): RuntimeAPI {
  ...
  return {
    ...
    async getBalance(address: string): Promise<BalanceResult> {
      const validAddr = expectAddress(address, 'address')
      ...
      if (jejuTokenAddress) {
        const tokenAddr = expectAddress(jejuTokenAddress, 'JEJU_TOKEN_ADDRESS')
      ...
      if (stakingManagerAddress) {
        const stakingAddr = expectAddress(
    ...
    async getServiceState(serviceId: string): Promise<ServiceState> {
      ...
      const raw: ServiceState = runningServices.get(serviceId) ?? {
        running: false,
        uptimeSeconds: 0,
        requestsServed: 0,
        earningsWei: 0n,
    ...
    async getBotState(botId: string): Promise<BotState> {
      const raw: BotState = runningBots.get(botId) ?? {
        running: false,
        uptimeSeconds: 0,
        opportunitiesDetected: 0,
        opportunitiesExecuted: 0,
...
async function createTauriAPI(): Promise<RuntimeAPI> {
  ...
  return {
    async detectHardware(): Promise<HardwareInfo> {
      ...
      return validateHardwareInfo(raw)
    },
  ...
  return {
    ...
    async connectWallet(): Promise<WalletConnection | null> {
      if (config.privateKey) {
        ...
        const validKey = expectHex(config.privateKey, 'private key in config')
        const client = createNodeClient(config.rpcUrl, config.chainId, validKey)
    ...
    async getBalance(address: string): Promise<BalanceResult> {
      const validAddress = expectAddress(address, 'balance query address')
      const client = createNodeClient(config.rpcUrl, config.chainId)
