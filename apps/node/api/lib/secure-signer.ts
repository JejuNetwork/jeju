...
import { getKMSMpcUrl, getKMSUrl, getCurrentNetwork } from '@jejunetwork/config'
...
export class SecureSigner {
  ...
  constructor(config: Partial<SecureSignerConfig> = {}) {
    ...
    this.config = {
      ...
      requireTeeAttestation: config.requireTeeAttestation ?? network !== 'localnet',
  ...
  async signMessage(request: SignMessageRequest): Promise<Hex> {
    ...
    const parsed = expectValid(SignResponseSchema, response, 'KMS sign response')
