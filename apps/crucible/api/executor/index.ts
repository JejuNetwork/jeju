...
import { asAddressOrDefault } from '../../lib/type-guards'
...
const config: CrucibleConfig = {
  ...
  contracts: {
    agentVault: asAddressOrDefault(process.env.AGENT_VAULT_ADDRESS, '0x0'),
    roomRegistry: asAddressOrDefault(process.env.ROOM_REGISTRY_ADDRESS, '0x0'),
    triggerRegistry: asAddressOrDefault(
      process.env.TRIGGER_REGISTRY_ADDRESS,
      '0x0',
    ),
    identityRegistry: asAddressOrDefault(
      process.env.IDENTITY_REGISTRY_ADDRESS,
      '0x0',
    ),
    serviceRegistry: asAddressOrDefault(
...
class ExecutorDaemon {
  ...
  private account = privateKeyToAccount(
    asAddressOrDefault(config.privateKey, '0x0'),
  )
