...
export async function startEdgeNode(): Promise<EdgeNodeServer> {
  ...
  const kmsKeyId = getEnvVar('CDN_EDGE_KMS_KEY_ID') ?? process.env.CDN_EDGE_KMS_KEY_ID
  const ownerAddress = (getEnvVar('CDN_EDGE_OWNER_ADDRESS') ?? process.env.CDN_EDGE_OWNER_ADDRESS) as Address | undefined
  const directKey =
