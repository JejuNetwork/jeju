...
/** Agent data from IdentityRegistry for TEE nodes */
interface TEEAgentData {
  owner: Address
  stakedAmount: bigint
  isBanned: boolean
...
/** Workload launch response */
interface WorkloadLaunchResponse {
  instanceId: string
...
export class RegionalTEECoordinator {
  ...
  private async deployToNode(
    ...
    const result = (await response.json()) as WorkloadLaunchResponse
