  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (nodeCount === 0) {
        ...
        console.warn('║                                                               ║')
        console.warn('║  To register a node, run:                                     ║')
        console.warn('║    bun scripts/register-node.ts --role miner                  ║')
        console.warn('║                                                               ║')
        console.warn('║  Or register via IdentityRegistry contract:                   ║')
        console.warn('║    1. Call register() to get an agentId                       ║')
        console.warn('║    2. Call addTag(agentId, "dws-worker-node")                 ║')
        console.warn('║    3. Set endpoint via setA2AEndpoint(agentId, endpoint)      ║')
        console.warn(
          '╚═══════════════════════════════════════════════════════════════╝',
        )
      } else {
        console.log(`[WorkerRouter] Discovered ${nodeCount} worker nodes from on-chain registry`)
      }
    } catch (error) {
      ...
      console.error('║  1. IdentityRegistry contract not deployed                    ║')
      console.error('║  2. Contract missing getAgentsByTag() function                ║')
      console.error('║  3. RPC node not accessible                                   ║')
      console.error('║                                                               ║')
      console.error('║  To fix:                                                      ║')
      console.error('║  - Ensure anvil/chain is running: anvil -p 6546               ║')
      console.error('║  - Deploy contracts: bun run deploy:localnet                  ║')
      console.error('║  - Register node: bun scripts/register-node.ts                ║')
      console.error(
        '╚═══════════════════════════════════════════════════════════════╝',
      )
