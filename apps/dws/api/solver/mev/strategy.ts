...
export class ExternalChainMevEngine extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `   Arbitrage:         ${this.config.enableArbitrage ? 'enabled' : 'disabled'}`,
    )
    console.log(
      `   Sandwich:          ${this.config.enableSandwich ? 'enabled' : 'disabled'}`,
    )
    console.log(
      `   Backrun:           ${this.config.enableBackrun ? 'enabled' : 'disabled'}`,
    )
    console.log(
      `   Liquidations:      ${this.config.enableLiquidations ? 'enabled' : 'disabled'}`,
    )
    console.log(
    ...
    console.log(
      `   MEV-Boost:         ${this.config.enableMevBoost ? 'enabled' : 'disabled'}`,
    )
    console.log(
      `   BuilderNet:        ${this.config.enableBuilderNet ? 'enabled' : 'disabled'}`,
    )
    console.log(
      `   Protect RPC:       ${this.config.enableProtect ? 'enabled (for Jeju users)' : 'disabled'}`,
    )
    ...
    console.log('\n[MEV] External chain MEV engine started')
  ...
  private async executeSandwich(
    ...
    if (!simulation.success) {
      console.log(`[MEV] Simulation failed`)
    ...
    if (simulation.totalProfit < this.config.minProfitWei) {
      console.log(
        `[MEV] Simulated profit too low: ${formatEther(simulation.totalProfit)} ETH`,
      )
      return
    }
    ...
    if (successCount > 0) {
      ...
      console.log(`[MEV] Submitted to ${successCount}/${results.size} builders`)
      console.log(
        `   Expected profit: ${formatEther(simulation.totalProfit)} ETH`,
      )
    } else {
      this.stats.bundlesFailed++
      console.log(`[MEV] All builder submissions failed`)
    }
  }
  ...
  private async executeBackrun(
    ...
    if (result.success) {
      ...
      console.log(`[MEV] Backrun submitted: ${result.bundleHash}`)
    } else {
      this.stats.bundlesFailed++
    }
  }
