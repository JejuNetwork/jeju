...
import type {
  ...
  CallBundleResult,
  CancelTxRpcResponse,
  FlashbotsRpcResponse,
  L2BlockSubmitResult,
  ProtectedTxRpcResponse,
  ProtectedTxStatusResult,
  SuaveRequestResult,
} from '../../types'
...
export class MevBoostProvider extends EventEmitter {
  ...
  async submitProtected(
    ...
    const result = (await response.json()) as ProtectedTxRpcResponse
  ...
  async cancelProtected(txHash: Hash): Promise<boolean> {
    ...
    const result = (await response.json()) as CancelTxRpcResponse
  ...
  async getProtectedStatus(txHash: Hash): Promise<{
    ...
    const result =
      (await response.json()) as FlashbotsRpcResponse<ProtectedTxStatusResult>
  ...
  async submitBundle(bundle: FlashbotsBundle): Promise<{ bundleHash: Hash }> {
    ...
    const result =
      (await response.json()) as FlashbotsRpcResponse<BundleSubmitResult>
  ...
  async simulateBundle(bundle: FlashbotsBundle): Promise<BundleSimulation> {
    ...
    const result =
      (await response.json()) as FlashbotsRpcResponse<CallBundleResult>
  ...
  async getBundleStats(
    ...
    const result =
      (await response.json()) as FlashbotsRpcResponse<BundleStatsResult>

    if (result.error) {
      throw new Error(`getBundleStats error: ${result.error.message}`)
    }

    return result.result as BundleStatsResult
  }
  ...
  async submitL2Block(
    ...
    const result =
      (await response.json()) as FlashbotsRpcResponse<L2BlockSubmitResult>
  ...
  async submitSuaveBundle(bundle: SuaveBundle): Promise<{ requestId: Hash }> {
    ...
    const result =
      (await response.json()) as FlashbotsRpcResponse<SuaveRequestResult>
  ...
  async getCurrentBlock(): Promise<bigint> {
    ...
    const result = (await response.json()) as FlashbotsRpcResponse<string>
