...
import { isCqlQueryResponse } from '../../shared/utils/type-guards'
import type {
  AgentCharacter,
  AgentMemory,
  AgentMessage,
...
function isInferenceResponse(data: unknown): data is InferenceApiResponse {
  if (typeof data !== 'object' || data === null) return false
  return true // Defensive - we access properties safely below
}

function isInvokeRequest(data: unknown): data is InvokeRequest {
  if (typeof data !== 'object' || data === null) return false
  const obj = data as Record<string, unknown>
  return obj.type === 'chat' || obj.type === 'think' || obj.type === 'cron'
}
...
Generate a brief internal thought or reflection about your recent experiences.`
  ...
  private async storeMemory(
    ...
    await fetch(`${this.env.DWS_CQL_URL}/api/v1/query`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  private async getRecentMemories(limit: number): Promise<AgentMemory[]> {
    ...
    const response = await fetch(`${this.env.DWS_CQL_URL}/api/v1/query`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        database: this.env.MEMORIES_DB_ID,
    ...
    const data: unknown = await response.json()
    if (!isCqlQueryResponse<AgentMemory>(data)) {
      return []
    }
    return data.rows ?? []
  }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/invoke' && request.method === 'POST') {
      ...
      try {
        ...
        if (!isInvokeRequest(rawBody)) {
          return new Response(
            JSON.stringify({
              success: false,
              error:
                'Invalid request format: type must be chat, think, or cron',
            }),
            {
              status: 400,
              headers: { 'Content-Type': 'application/json' },
...
function isCharacterRow(row: unknown): row is CharacterRow {
...
async function loadCharacterFromCQL(env: Env): Promise<AgentCharacter> {
  ...
  const response = await fetch(`${env.DWS_CQL_URL}/api/v1/query`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      database: 'agents',
  ...
  const data: unknown = await response.json()
  if (!isCqlQueryResponse<CharacterRow>(data)) {
    throw new Error('Invalid CQL response format')
  }
  ...
  if (!isCharacterRow(firstRow)) {
    throw new Error('Invalid character row format')
  }

  return JSON.parse(firstRow.character)
}
