...
function isPullRequestsResponse(data: unknown): data is PullRequestsResponse {
  return hasArrayProperty(data, 'pulls')
}

function isPullRequest(data: unknown): data is PullRequest {
  return (
    isRecord(data) &&
...
async function fetchPullRequests(query?: {
  ...
  if (!isPullRequestsResponse(data)) return []
  return data.pulls
}
...
async function createPullRequest(data: {
  ...
  if (!isPullRequest(result)) return null
  return result
}
