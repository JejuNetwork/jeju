...
interface BountiesResponseItem {
  id: string
  title: string
  status: string
  reward: string
...
function isValidBountyStatus(
  status: string,
): status is ProfileBounty['status'] {
  return ['open', 'in_progress', 'review', 'completed'].includes(status)
}
...
async function fetchUserBounties(address: string): Promise<ProfileBounty[]> {
  ...
  return data.bounties.map((b) => ({
    id: b.id,
    title: b.title,
    status: isValidBountyStatus(b.status) ? b.status : 'open',
...
function isApiRepo(item: unknown): item is ApiRepo {
  if (!isRecord(item)) return false
  return typeof item.name === 'string'
}

function isApiRepoArray(data: unknown): data is ApiRepo[] {
  return Array.isArray(data) && data.every(isApiRepo)
}

async function fetchUserRepos(owner: string): Promise<ProfileRepo[]> {
  const response = await api.api.git.get({ query: { owner } })
  const data = extractDataSafe(response)
  if (!isApiRepoArray(data)) return []
  return data.map((r) => ({
    name: r.name,
