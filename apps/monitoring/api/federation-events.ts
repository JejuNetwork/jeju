#!/usr/bin/env bun
/** Federation contract event monitor - watches RegistryHub, NetworkRegistry, CrossChainIdentitySync. */
...
/** Extended Log type that includes decoded event name from watchContractEvent. */
interface DecodedLog extends Log {
  eventName?: string
}

/** Type guard for checking if a log has been decoded with eventName. */
function hasEventName(log: Log): log is DecodedLog & { eventName: string } {
  return 'eventName' in log && typeof (log as DecodedLog).eventName === 'string'
}

/** Extract event info from a log, with fallback for eventName. */
function getLogEventInfo(log: Log): {
  log: DecodedLog
  eventName: string
  blockNumber: bigint
} {
  ...
  return {
    log: decodedLog,
    eventName: hasEventName(log) ? log.eventName : 'Unknown',
    blockNumber: decodedLog.blockNumber ?? 0n,
  }
}
...
/** Validate and parse an Ethereum address from environment variable. */
function parseEnvAddress(envVar: string | undefined): Address | undefined {
  if (!envVar) return undefined
  if (!isAddress(envVar)) {
    console.warn(`⚠️ Invalid address format: ${envVar}`)
    return undefined
  }
  return envVar
}

const REGISTRY_HUB = parseEnvAddress(process.env.REGISTRY_HUB)
const NETWORK_REGISTRY = parseEnvAddress(process.env.NETWORK_REGISTRY)
const CROSS_CHAIN_SYNC = parseEnvAddress(process.env.CROSS_CHAIN_SYNC)
    ...
    client.watchContractEvent({
      ...
      onLogs: async (logs) => {
        for (const log of logs) {
          ...
          await handleRegistryHubEvent(decodedLog, eventName)
        }
      },
    })
  }
    ...
    client.watchContractEvent({
      ...
      onLogs: async (logs) => {
        for (const log of logs) {
          ...
          await withMetricsLock(() => updateLastBlock(blockNumber))
          await handleNetworkRegistryEvent(decodedLog, eventName)
        }
      },
    })
  }
    ...
    client.watchContractEvent({
      ...
      onLogs: async (logs) => {
        for (const log of logs) {
          ...
          await withMetricsLock(() => updateLastBlock(blockNumber))
          await handleCrossChainSyncEvent(decodedLog, eventName)
        }
      },
    })
  }
