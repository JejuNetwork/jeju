...
describe('Trade Amount Validation', () => {
  ...
  test('should reject zero amount', () => {
    expect(() => validateTradeAmount(0n)).toThrow('Trade amount must be positive');
  });

  test('should reject negative amount', () => {
    expect(() => validateTradeAmount(-1n)).toThrow('Trade amount must be positive');
  });
...
describe('Calculate Minimum Shares', () => {
  ...
  test('should throw for invalid slippage', () => {
    ...
    expect(() => calculateMinShares(expectedShares, 6000n)).toThrow('Slippage cannot exceed 50%');
  });
});
