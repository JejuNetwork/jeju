...
import { CHAIN_ID, CONTRACTS, RPC_URL } from '../config'

// Default paymaster config for Bazaar
const DEFAULT_PAYMASTER_CONFIG: PaymasterConfig = {
  factoryAddress: (CONTRACTS.bazaar ||
    '0x0000000000000000000000000000000000000000') as Address,
  minStakedEth: BigInt(1e16), // 0.01 ETH
  rpcUrl: RPC_URL,
  chainId: CHAIN_ID,
}
...
export default function PaymasterSelector({
  ...
  onSelect,
  className = '',
  config = DEFAULT_PAYMASTER_CONFIG,
  ...
  const loadPaymasters = useCallback(async () => {
    setLoading(true)
    const paymasterOptions = await getPaymasterOptions(
      estimatedGas,
      gasPrice,
      config,
    )
    ...
    setLoading(false)
  }, [estimatedGas, gasPrice, onSelect, config])
