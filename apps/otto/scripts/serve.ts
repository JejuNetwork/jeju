...
async function main() {
  const host = getLocalhostHost()
  console.log('[Otto] Starting production server...')
  ...
  console.log(`[Otto] Starting server on port ${API_PORT}...`)

  process_ref = Bun.spawn(['bun', 'run', 'dist/server.js'], {
    cwd: APP_DIR,
    stdout: 'inherit',
    stderr: 'inherit',
    env: {
  ...
  if (!ready) {
    console.error('[Otto] Failed to start server')
    cleanup()
    process.exit(1)
  }
  ...
  console.log('')
  console.log('╔════════════════════════════════════════════════════════════╗')
  console.log('║            Otto Production Server Ready                     ║')
  console.log('╠════════════════════════════════════════════════════════════╣')
  console.log(
