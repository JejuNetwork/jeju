...
export default function ReputationViewer({ agentId }: ReputationViewerProps) {
  ...
  const { data: agentData, isLoading: loadingAgent } = useReadContract({
    address: MODERATION_CONTRACTS.IdentityRegistry,
  ...
  const { data: isNetworkBanned, isLoading: loadingBan } = useReadContract({
    address: MODERATION_CONTRACTS.BanManager,
    abi: BAN_MANAGER_ABI,
  ...
  const { data: labelIds, isLoading: loadingLabels } = useReadContract({
    address: MODERATION_CONTRACTS.ReputationLabelManager,
  ...
  useEffect(() => {
    if (agentData && typeof isNetworkBanned === 'boolean') {
      const labelArray = Array.isArray(labelIds) ? labelIds : []
      const labels = labelArray.map((id) => getLabelName(Number(id)))
      const appBanArray = Array.isArray(appBans) ? appBans : []
      const appBanNames = appBanArray.map(bytes32ToAppName)
