...
/**
 * Validates and returns an Address from an environment variable.
 * Returns ZERO_ADDRESS if not set or invalid.
 */
function getEnvAddress(envValue: string | undefined): Address {
...
const TESTNET_CONTRACTS: ContractAddresses = {
  banManager: getEnvAddress(process.env.PUBLIC_BAN_MANAGER),
  moderationMarketplace: getEnvAddress(
    process.env.PUBLIC_MODERATION_MARKETPLACE,
  ),
  reputationLabelManager: getEnvAddress(
    process.env.PUBLIC_REPUTATION_LABEL_MANAGER,
  ),
  reportingSystem: getEnvAddress(process.env.PUBLIC_REPORTING_SYSTEM),
  predimarket: getEnvAddress(process.env.PUBLIC_PREDIMARKET),
  registryGovernance: getEnvAddress(process.env.PUBLIC_REGISTRY_GOVERNANCE),
  identityRegistry: getEnvAddress(process.env.PUBLIC_IDENTITY_REGISTRY),
  evidenceRegistry: getEnvAddress(process.env.PUBLIC_EVIDENCE_REGISTRY),
  reputationProviderRegistry: getEnvAddress(
    process.env.PUBLIC_REPUTATION_PROVIDER_REGISTRY,
  ),
}
