...
import type { JsonArray, JsonRecord, JsonValue } from '@jejunetwork/sdk'
...
interface A2ASkillParams {
  [key: string]: JsonValue | JsonRecord | JsonArray
}
...
export const ceoA2A = {
  ...
  chat: (message: string) => ceoA2A.call<JsonRecord>('chat', { message }),
}
...
export async function fetchAgentCard(baseUrl: string): Promise<{
  ...
  return expectValidResponse(response, AgentCardSchema, 'Agent card')
}
