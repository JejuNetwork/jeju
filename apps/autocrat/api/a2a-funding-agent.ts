...
function requireVote(
  ...
  if (value !== 'APPROVE' && value !== 'REJECT' && value !== 'ABSTAIN') {
    throw new Error('vote must be APPROVE, REJECT, or ABSTAIN')
  }
  return value
}
...
async function handleSkill(
  ...
  switch (skillId) {
    case 'get_funding_pool': {
      const daoId = requireString(input.daoId, 'daoId')
    ...
    case 'get_contributor_recommendations': {
      const daoId = requireString(input.daoId, 'daoId')
      const result = await fundingApi.generateContributorRecommendations(daoId)
      if (!result.success || !result.data) {
    ...
    case 'vote_weight': {
      ...
      const adjustment = requireNumber(input.adjustment, 'adjustment')
      const reason = requireString(input.reason, 'reason')
      const reputation = requireNumber(input.reputation, 'reputation')
    ...
    case 'review_payment_request': {
      ...
      const vote = requireVote(input.vote)
      const reason = requireString(input.reason, 'reason')
      const result = await fundingApi.councilVote(requestId, vote, reason)
      if (!result.success) {
    ...
    case 'get_contributor_profile': {
      const contributorId = requireString(input.contributorId, 'contributorId')
...
const A2ATaskRequestBodySchema = t.Object({
  ...
  input: t.Object({
    ...
    adjustment: t.Optional(t.Number()),
    reason: t.Optional(t.String()),
    reputation: t.Optional(t.Number()),
...
export function createA2AFundingServer() {
  const app = new Elysia()
    ...
    .post(
      '/tasks',
      async ({ body }): Promise<A2ATaskResponse> => {
