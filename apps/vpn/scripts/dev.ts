#!/usr/bin/env bun
/**
 * VPN Development Server
 *
 * Starts both API and frontend with hot reload:
 * - API: Bun with --watch on port 1422
 * - Frontend: Dev server with HMR on port 1421
 */
...
const API_PORT = Number(process.env.API_PORT) || 1422
const FRONTEND_PORT = Number(process.env.PORT) || 1421
...
async function startAPIServer(): Promise<boolean> {
  console.log(`[VPN] Starting API server on port ${API_PORT}...`)
  ...
  if (!ready) {
    console.error('[VPN] Failed to start API server')
    return false
  }

  console.log(`[VPN] API server started on port ${API_PORT}`)
  return true
}
...
async function buildFrontend(): Promise<boolean> {
  ...
  console.log(`[VPN] Frontend built in ${duration}ms`)
  return true
}

async function startFrontendServer(): Promise<boolean> {
  console.log(`[VPN] Starting frontend dev server on port ${FRONTEND_PORT}...`)

  await mkdir(resolve(APP_DIR, 'dist/dev'), { recursive: true })

  const buildSuccess = await buildFrontend()
  if (!buildSuccess) {
    console.error('[VPN] Initial frontend build failed')
    return false
  }
    ...
    async fetch(req) {
      ...
      if (pathname.startsWith('/api') || pathname === '/health') {
        ...
        try {
          ...
          console.error('[VPN] Proxy error:', (error as Error).message)
          return new Response(
...
async function main() {
  const host = getLocalhostHost()
  console.log('╔════════════════════════════════════════════════════════════╗')
  console.log('║              VPN Development Server                         ║')
  console.log('╚════════════════════════════════════════════════════════════╝')
  console.log('')
