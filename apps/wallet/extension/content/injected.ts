...
function isResponseMessage(data: { type: string }): data is ResponseMessage {
  return (
    data.type === 'jeju_response' &&
    'id' in data &&
    typeof (data as ResponseMessage).id === 'string'
  )
}

// Type guard for EventMessage
function isEventMessage(data: { type: string }): data is EventMessage {
  return data.type === 'jeju_event' && 'event' in data && 'data' in data
}
...
class NetworkProvider {
  ...
  private async initialize(): Promise<void> {
    ...
    if (typeof chainId === 'string' && isHex(chainId)) {
