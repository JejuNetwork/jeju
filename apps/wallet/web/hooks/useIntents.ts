...
import { chains } from '../../api/sdk/chains'
import { OIFClient } from '../../api/sdk/oif'
import type {
  Intent,
  IntentParams,
  IntentQuote,
...
import { IntentHistoryResponseSchema } from '../../lib/api-responses'
  ...
  const refundMutation = useMutation({
    ...
    onSuccess: (_, intentId) => {
      setActiveIntents((prev) =>
        prev.map((intent) =>
          intent.id === intentId
            ? { ...intent, status: 'expired' satisfies IntentStatus }
            : intent,
        ),
      )
