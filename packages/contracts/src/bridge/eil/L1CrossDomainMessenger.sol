 ...
 * @notice L1 side of the cross-domain messaging system for local development
 * @dev Emits SentMessage events that the relay service picks up and delivers to L2
 */
contract L1CrossDomainMessenger {
    /// @notice The L2 messenger address (set after deployment)
    ...
    function setL2Messenger(address _l2Messenger) external onlyOwner {
        l2Messenger = _l2Messenger;
    }

    /// @notice Send a cross-domain message to L2
    /// @param _target Target contract on L2
    ...
    /// @notice Relay a message from L2
    /// @dev Called by the relay service with the sender set to the L2 contract
