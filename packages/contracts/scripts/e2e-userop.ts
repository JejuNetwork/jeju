...
import { createPublicClient, createWalletClient, http, encodeFunctionData, parseEther, keccak256, encodeAbiParameters, parseAbiParameters, toHex, concat, pad, numberToHex, type Hex } from 'viem';
...
const USER_PRIVATE_KEY = '0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a'; // Account 2
...
async function main() {
  ...
  console.log(`   Paymaster deposit: ${paymasterDeposit} wei (${Number(paymasterDeposit) / 1e18} ETH)`);
  ...
  if (result.error) {
    ...
    if (!accountExists) {
      console.log('   Account not deployed yet - this is expected for first UserOp');
    }
