...
function detectArtifactPlatform(filename: string): {
...
async function uploadToDWS(data: Buffer, filename: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `Failed to connect to DWS at ${dwsUrl}. Is DWS running? Error: ${err instanceof Error ? err.message : 'Unknown error'}`,
    )
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to upload to DWS: ${response.status} ${response.statusText} - ${body}`,
    )
  }
...
async function fetchReleaseIndex(appName: string): Promise<ReleaseIndex> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to fetch release index: ${response.status} ${response.statusText}`,
    )
  }
...
async function publishReleaseIndex(
  appName: string,
  index: ReleaseIndex,
): Promise<void> {
  const dwsUrl = getDWSUrl()
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => 'unknown error')
    throw new Error(
      `Failed to publish release index: ${response.status} ${response.statusText} - ${body}`,
    )
  }
}
  ...
  const scanDir = (dir: string) => {
    ...
    for (const entry of readdirSync(dir)) {
      ...
      if (stat.isDirectory()) {
        ...
        if (
          ['dmg', 'msi', 'exe', 'appimage', 'deb', 'rpm'].includes(ext ?? '')
        ) {
          ...
          mkdirSync(join(config.releaseDir, config.version), {
            recursive: true,
          })
...
async function buildExtension(
  ...
  for (const browser of ['chrome', 'firefox', 'edge'] as const) {
    const outputFile = join(
      config.releaseDir,
      config.version,
      `${config.app}-${browser}-${config.version}.zip`,
    )
    ...
    await execa(
      'zip',
      ['-r', outputFile, '.', '-x', 'node_modules/*', '-x', 'scripts/*'],
      {
        cwd: extensionDir,
        stdio: 'inherit',
      },
...
async function publishArtifacts(
  ...
  for (const artifact of artifacts) {
    ...
    logger.info(
      `Uploading ${artifact.filename} (${(data.length / 1024 / 1024).toFixed(2)} MB)...`,
    )
...
releaseCommand
  ...
  .action(
    async (
      appName: string,
      options: { version?: string; platform?: string },
    ) => {
      ...
      if (
        hasExtension &&
        (!options.platform ||
          ['chrome', 'firefox', 'edge'].includes(options.platform))
      ) {
      ...
      artifacts.forEach((a) => {
        logger.info(
          `  - ${a.filename} (${a.platform}${a.arch ? `/${a.arch}` : ''})`,
        )
      })
      logger.info('')
    ...
    async (
      ...
      if (!version) {
        ...
        if (versions.length === 0) {
          logger.error(
            'No built releases found. Run `jeju release build` first.',
          )
          process.exit(1)
        }
        version = versions[0]
      ...
      // Upload artifacts
      const publishedArtifacts = await publishArtifacts(
        config,
        localManifest.artifacts,
      )
      ...
      logger.info(
        `Download URL: ${getDWSUrl()}/storage/releases/${appName}/${version}/manifest.json`,
      )
    },
  )
...
releaseCommand
  ...
  .action(async (appName: string) => {
    ...
    if (index.latestNightly)
      logger.info(`  Latest nightly: ${index.latestNightly}`)
    logger.info('')
    logger.info('  All versions:')
    for (const v of index.versions.slice(0, 10)) {
      logger.info(
        `    - ${v.version} (${v.channel}) - ${new Date(v.releasedAt).toLocaleDateString()}`,
      )
    }
...
releaseCommand
  ...
  .action(async (appName: string, version?: string) => {
    ...
    const response = await fetch(
      `${dwsUrl}/storage/releases/${appName}/${targetVersion}/manifest.json`,
    )
    if (!response.ok) {
    ...
    for (const artifact of manifest.artifacts) {
      ...
      logger.info(
        `    Platform: ${artifact.platform}${artifact.arch ? ` (${artifact.arch})` : ''}`,
      )
