...
function analyzeTransactionData(
  ...
  if (valueInEth > 10) {
    ...
    recommendations.push('Verify recipient address before sending large amounts')
  }
...
async function analyzeContractBytecode(
  ...
  const chainConfig = getChainConfig(network as 'localnet' | 'testnet' | 'mainnet')
  ...
  if (!code || code === '0x') {
    return { hasCode: false, codeSize: 0, isProxy: false, warnings: ['Address has no contract code - EOA or destroyed'] }
  }
  ...
  if (codeSize < 100) {
    warnings.push('Very small contract - may be a proxy or minimal implementation')
  }
  ...
  if (hardcodedAddresses && hardcodedAddresses.length > 5) {
    warnings.push(`Contract contains ${hardcodedAddresses.length} hardcoded addresses`)
  }
...
export const analyzeTransactionAction: Action = {
  ...
  handler: async (
    ...
    if (!addressMatch) {
      callback?.({ text: 'Please provide a valid Ethereum address to analyze.' })
      return
    }
    ...
    const value = valueMatch ? (parseFloat(valueMatch[1]) * 1e18).toString() : '0'
...
export const scanContractAction: Action = {
  name: 'SCAN_CONTRACT',
  description: 'Scan a smart contract for security vulnerabilities and red flags',
