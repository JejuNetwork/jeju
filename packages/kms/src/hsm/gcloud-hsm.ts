...
export class GCloudHSMProvider implements HSMProvider {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      log.error('GCloud HSM connection failed', { error: error instanceof Error ? error.message : String(error) })
  ...
  async getKey(keyId: string): Promise<HSMKeyRef | null> {
    ...
    try {
      ...
      log.error('Failed to get GCloud HSM key', { keyId, error: error instanceof Error ? error.message : String(error) })
      return null
    }
  }
  ...
  async deriveKey(
    ...
    const ciphertextBytes = typeof encrypted.ciphertext === 'string' 
      ? new TextEncoder().encode(encrypted.ciphertext) 
      : encrypted.ciphertext
    const hash = await crypto.subtle.digest('SHA-256', ciphertextBytes.buffer as ArrayBuffer)
